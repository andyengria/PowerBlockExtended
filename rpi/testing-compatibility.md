...
To test backward-compatibility across the three mechanisms for GPIO access on Raspberry Pi, we target a combination of hardware and software setups that cover the following:
...
- Raspberry Pi 4 Model B: This is the most current and widely used Raspberry Pi model, running the latest distributions of Raspberry Pi OS. It will give the best insight into how the tool works with libgpiod v2.
...
- Raspberry Pi Zero W: While this is a smaller form factor, it often runs older distributions of Raspberry Pi OS, which would helps test sysfs fallback. It is still a valid platform for GPIO projects, and it provides a test environment for the sysfs method.
...
- libgpiod v2 (default).
- Check if tool works with the latest GPIO access method.
...
- This should give the best coverage of legacy hardware and software.
